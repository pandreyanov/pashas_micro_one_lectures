...
$$\max_{\tau_x, \tau_y} V(p,q,I) - \mu (\tau_x x(p) + \tau_y y(p) - T) $$
...
$$ \mathcal{L} = U(x, y) - \lambda (p x + q y - I) \quad \Rightarrow \quad \frac{\partial V}{\partial p} = - \lambda x, \quad \frac{\partial V}{\partial q} = - \lambda y$$
