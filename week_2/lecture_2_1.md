...
В большей часть примеров, которые мы будем рассматривать, спросы будут выражаться через элементарные функции, такие как $x^2, \log x, 1/x$... Все эти функции непрерывны. Является ли это совпадением? Оказывается что нет.

:::{admonition} **Теорема Максимума**
:class: warning
В строго выпуклой задаче оптимизации, решение, если оно существует, то единственно. Более того, если задача непрерывна по параметрам, то как координаты решения так и значение целевой фунцкии непрерывны по параметру.
:::
