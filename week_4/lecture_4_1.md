...
Предположим, опять, смещение цен $p \to p'$ и что полезность агентов упала до уровня $\bar U_1$. Определим эквивалентную вариацию как изменение дохода, которое было бы эквивалентно этому смещению цен.

:::{prf:definition}

**Эквивалентная вариация** определяется как изменение в расходах, ассоциированных с новым уровнем полезности

$$CV = E(p',\bar U_1) - E(p,\bar U_1), \quad \bar U_1 = U(x^{\ast}(p'), y^{\ast}(p'))$$
