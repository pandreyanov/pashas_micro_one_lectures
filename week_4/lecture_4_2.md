...
Получается, что в последней формулировке, мы можем пользоваться квадратичным приближением $CV$ для поиска $IE$, и затем вычитать его из глобального изменения спроса ($TE$ = Total Effect), чтобы получить $SE$:
