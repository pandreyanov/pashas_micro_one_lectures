...
Осталось понять, что значит строгая выпуклость и непрерывность задачи. Для простоты мы сфокусируемся на более менее общей постановке в теории потребителя:

$$U(x,y) \to \max_{x,y \geqslant 0}, \quad s.t. B(x,y) \leqslant 0$$

:::{admonition} 
:class: tip
В **строго выпуклой задаче**, множества $L_{+}$ функции U выпуклы, и множества $L_{-}$ функции B выпуклы, причем одна из них строго. К примеру, достаточно, чтобы $B$ была линейна а $U$ строго вогнута.
:::
