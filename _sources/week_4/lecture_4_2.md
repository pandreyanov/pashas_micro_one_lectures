...
То есть, $CV$ и $EV$, в малых приращениях, это значение квадратичной формы матрицы Слуцкого на векторе приращения цен, разделенная пополам. Это уже довольно сложная технически теорема, поэтому только формулировка.

:::{prf:theorem} Зачем нужна $S$
:class: remark
Если полезность дважды дифференцируема, то верно следующее

$$CV \approx EV \approx \frac{\delta p \cdot S \cdot \delta p^T}{2},$$

при малых приращениях цен $\delta p = p' - p$.

:::

То есть, если разложить $CV$ или $EV$ в ряд Тейлора по $\delta p = p'-p$ то линейный член обнулится, а квадратичный член будет задаваться матрицей $S/2$. Это просто невероятно крутой космический результат, но вы, скорее всего, еще слишком молоды, чтобы это понять.
