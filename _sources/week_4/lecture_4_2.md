...
$$ E(p') - E(p) = \int_p^{p'} h(x) dx \approx (p'-p) h(p') = (p'-p)(h(p') - h(p) + h(p)) $$

То есть, $CV$ и $EV$, это, в линейном приближение скалярное произведение хиксианского спроса на приращение цен, а во втором приближении значение квадратичной формы матрицы Слуцкого на векторе приращения цен. Это уже довольно сложная технически теорема, поэтому только формулировка.

:::{prf:theorem} Зачем нужна $S$
:class: remark
Если полезность дважды дифференцируема, то верно следующее

$$CV \approx EV \approx \delta p \cdot \vec h + \delta p \cdot S \cdot \delta p^T,$$

при малых приращениях цен $\delta p = \vec p' - \vec p$, где $\vec h$ - действующий спрос.

:::

Это просто невероятно крутой космический результат, но вы, скорее всего, еще слишком молоды, чтобы это понять.
