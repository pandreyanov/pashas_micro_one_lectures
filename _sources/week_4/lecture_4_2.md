...
То есть, $CV$ и $EV$, в малых приращениях, это действие квадратичной формы матрицы слуцкого на вектор приращения цен, разделенная пополам. Это уже довольно сложная технически теорема, поэтому только формулировка.

:::{prf:theorem} Зачем нужна $S$
:class: remark
Если полезность дважды дифференцируема, то верно следующее

$$CV \approx EV \approx \frac{\delta p \cdot S \delta p^T}{2},$$
