...
:::{dropdown} Подсказка: обратите внимание на единицы измерения. Подумайте, прежде чем прочитать ответ.
...
:::{dropdown} Подсказка: воспользуемся разложением Тэйлора в единице $\log(1+x) \approx 1 + x$. Подумайте, прежде чем прочитать ответ. 

Связь между процентами и логарифмами следующая. Рассмотрим как логарифм меняется при небольшом приращении аргумента:

$$\frac{\log (x + \delta x)}{x} = \frac{\log (x/\delta x +  1)}{x/ \delta x} \approx \frac{x/\delta x + 1}{x/\delta x} = \frac{x + \delta x}{x}$$

Правая часть уравнения это, как раз, и есть относительное приращение. Умножив его на 100, мы получим классическое приращение в процентах.
:::
