...
Сформулируем Лагранжиан для задачи минимизации расходов:

$$\mathcal{L} = \alpha x^{\rho} + (1-\alpha)y^{\rho} - \lambda(p x + q y - I).$$ 

На самом деле, неважно, к которому из двух членов добавлять множитель лагранжа: к полезности или к бюджету. Поэтому я скопирую условия первого порядка из Лекции 2.

- $\alpha \rho x^{\rho} = \lambda p x$
- $(1-\alpha) \rho y^{\rho} = \lambda q y$

Единственное что изменилось это ограничение

- $\alpha x^{\rho} + (1-\alpha)y^{\rho} = \bar U$

Используя несложную подстановку, получаем: 

$$\lambda I = \lambda p x + \lambda q y = \bar U / \rho$$