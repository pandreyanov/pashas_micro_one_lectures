...
$$
\mathcal{L}^{P1} = U(x, y) - \lambda (px + qy - I)\\
\mathcal{L}^{P2} = (px + qy - I) - \gamma (\bar U - U(x,y))
$$

Поверим на слово, что обе задачи выпуклые, и выпишем УПП для поиска оптимума:

$$  
\text{P1:} U'_x = \lambda p, \quad U'_y = \lambda q, \quad px + qy = I\\
...
- во первых, $\gamma \lambda = 1$, что нас никак не ограничивает
- во вторых, при достижении таргетированной полезности обязательно расходовался весь бюджет и наоборот
...
Теперь мы готовы сформулировать теорему

:::{prf:theorem} Закон Вальраса
:class: remark
Если полезность локально ненасыщаема, то любое из решений любой задачи максимизации полезности всегда лежит на бюджетном ограничении.
:::
